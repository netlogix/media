		...
		try {
			$adjustments = array(
				new ResizeImageAdjustment(
					array(
						'maximumWidth' => $thumbnail->getMaximumWidth(),
						'maximumHeight' => $thumbnail->getMaximumHeight(),
						'ratioMode' => $thumbnail->getRatioMode(),
						'allowUpScaling' => $thumbnail->isUpscalingAllowed()
					)
				)
			);
